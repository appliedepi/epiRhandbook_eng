...
There are several extensive **ggplot2** tutorials linked in the resources section. You can also download this [data visualization with ggplot cheatsheet](https://rstudio.github.io/cheatsheets/data-visualization.pdf) from the RStudio website. If you want inspiration for ways to creatively visualise your data, we suggest reviewing websites like the [R graph gallery](https://www.r-graph-gallery.com/) and [Data-to-viz](https://www.data-to-viz.com/caveats.html). 
...
Faceting is a functionality that comes with **ggplot2**, so the legends and axes of the facet "panels" are automatically aligned. There are other packages discussed in the [ggplot tips](ggplot_tips.qmd) page that are used to combine completely different plots (with **patchwork**) into one figure.  
...
* Add a [complete theme](https://ggplot2.tidyverse.org/reference/ggtheme.html). The function `theme_()` makes sweeping adjustments - these include: `theme_classic()`, `theme_minimal()`, `theme_dark()`, `theme_light()` `theme_grey()`, `theme_bw()` among others.  
...
3. Within the `element_()` function, write argument assignments to make the fine adjustments you desire.  
...
Categorical data can be character values, could be logical (TRUE/FALSE), or factors (see the [Factors](factors.qmd) page). 
