...
# In real life you will not know the vaccination status of all the students: it could be the reason you are doing the survey, but here we control everything!
...
There is a risk of adding a bias without noticing with this strategy though. Using a regular sampling interval $k$ could oversample participants with specific characteristics if the sampling frame you use is ordered in a specific way.
...
# We still want to select 250 students so let us calculate the interval
...
You can see in this example that we are substantially overestimating the vaccination coverage based on our sample. It is because the sampling frame (the list of students) is ordered in a specific way and our sampling interval happens to be even. Because the criteria used to order the sampling frame is associated with our outcome of interest (boys had more chance to be vaccinated) we can oversample or undersample boys if our sampling interval is even if our first selected student is a girl or a boy. This toy example is just a cartoonish illustration.

In practice it can be very difficult to detect such issues before it is too late. The simple solution to this is to randmoly shuffle your sampling frame before making the systematic selection.
...
# Now let us select the 250 students so let us calculate the interval
interval <- nrow(student_list)/250
# Now the random beginning
beginning <- sample(1:interval, 1)
