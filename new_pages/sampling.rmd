...
sampled_points_df <- as(sampled_points, "Spatial") %>% # We turn our points into a SpatialPointsDataFrame and then a data.frame to use it outside of a map
						as.data.frame() %>%
						select(Longitude=coords.x1, Latitude=coords.x2) %>%
...
ggplot() +
	...
	geom_sf(
		...
		color="red") +
	coord_sf(
		datum = sf::st_crs(32605)) + # To project to UTM (see more in the GIS chapter) to have the axes in meters
	theme_bw()
```
...
# Here we use leaflet because of its interactivity.
...
In our example we sampled buildings with GPS positions already available thanks to OSM data. However, you might also need to draw points in polygons because such data are not available in your study area. In this case you will often see that a lot of points do not fall on roofs. Our current approach to check the points is convenient but only exploratory at best. In the situation where a lot of points are not on roofs, you will have to remove them. That also means that you will need to resample others and then clean them too. It could require the use of other GIS softwares (some are free and very efficient such as [QGIS](https://www.qgis.org/en/site/) or [GeoDa](https://geodacenter.github.io/)) and will also require some more GIS skills.

Sampling GPS point can be very convenient and powerful in areas where very little information is available to make a more traditional sampling frame or simply because you know the available information is unreliable. In areas with large population displacements, it can be easier to find **up-to-date satellite imageries** of the area to draw boundaries and sample households than waiting the demographic data to catch-up with the current situation. However, there are some limitations you should also be aware of:
...
* Selecting households by drawing pointsmeans that a house has as much chance to be selected as a skyscraper with a roof of the same size. People living in the skyscraper would then have a lower selection probability than people in the house if you randomly choose them. So, either you collect data on the number of people/family living in the selected house/skyscraper to weigh the data accordingly and correct for this unequal selection probability or you just avoid using this sampling strategy if there is a strong heterogeneity in the number of floors in the area.
* Because the points your draw are randomly selected you might have by chance either very few points on roofs or sometimes more than one on the same roof. You need to decide on a rule that you apply consistently during your "cleaning" phase for those two situations:
	...
	* To minimize the number of points discarded because they are not exactly on a roof, it is not uncommon to use a buffer around the points (20 meters for example could be reasonable). If a roof is within this buffer, then you consider that house as selected. But there is drawback: the bigger the buffer the more isolated households are likely to be selected. Besides, when adding a buffer, more than 1 household might fall in the buffer and you need to decide on a rule to decide which one is selected. Adding a buffer is common. So, deciding on its size is a matter of trade-off (again).
...
This strategy also has logistical advantages: you have the coordinates of all the households you want to select. You just need to put them in GPS devices and dispatch your teams in an organized fashion. You can also SHOW them where they should go using softwares such as [Google Earth](https://www.google.com/intl/fr/earth/versions/#earth-pro). So the practical management of data collection is often simpler. You will need to juggle between different data formats if you want to export those points in GPS devices or smartphones/tablets using a a GIS application. Two of the common formats you shouls be aware of include gpx (commonly used by some GPS devices and some apps such as [maps.me](https://maps.me/)) and kml (commonly used in other apps such as [OsmAnd](https://osmand.net/) or software like [Google Earth](https://www.google.com/intl/fr/earth/versions/)).

```{r "editing kml ang gpx", echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
# Here is ho you could save your sampled points into kml points.
sampled_points %>%
...
# We will convert the sf data into SpatialPointsDataFrame, part of the sp format, as an intermediate step before saving them as gpx.
# Some packages not on the CRAN would allow you to skip that step, but we will stick to options available on the CRAN.
sampled_points_sp <-   as(sampled_points, "Spatial") # Here is the conversion to SpatialPointsDataFrame
# Some functionalities are sometimes more convenient using the sp format so some familiarity with it could be useful.
# The main difference you might keep in mind between sf and sp formats is that you could see sf data as data.frames with geometry attributes, whereas sp data could be seen as geometry elements with data attributes.
