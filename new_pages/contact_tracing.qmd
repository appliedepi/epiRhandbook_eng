...
You can read more about the Go.Data project on the [Github Documentation site](https://worldhealthorganization.github.io/godata/). 
...
In our example data, the only records that this applies to are ones missing an ID! We can remove those. But, for purposes of demonstration we will go show the steps for de-duplication so there is only one follow-up encounter per person per day. See the page on [De-duplication](deduplication.qmd) for more detail. We will assume that the most recent encounter record is the correct one. We also take the opportunity to clean the `followup_number` column (the "day" of follow-up which should range 1 - 14).  
...
[Go.Data](https://worldhealthorganization.github.io/godata/)

[Automated R Reporting using Go.Data API](https://github.com/WorldHealthOrganization/godata/tree/master/analytics/r-reporting)
