...
The function `pmap()` from the **purrr** package allows us to apply `map_*()` functions over multiple vectors. The "p" in `pmap()` stands for parallel. It works down a a dataset or list sequentially, carrying out your operation. Note, it does **not** refer to parallel [computing](https://bookdown.org/rdpeng/rprogdatascience/parallel-computation.html).
...
As another example, here we have written our own [custom function](writing_functions.qmd), using `str_glue`, see section on [Characters and strings](characters_strings.qmd) to summarise each patient's gender, age, date of onset, outcome and the date of outcome: 
