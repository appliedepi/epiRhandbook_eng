---
title: "Outbreak dashboard (Shiny)"
output: 
...
```{r}
hospital_choices <- rev(unique(linelist$hospital))   # create vector of hospital values to choose from

# Create drop-down menu of hospitals. Selection saved as 'hospital_choice'
selectInput("hospital_choice", label = "Display Epicurve of cases admitted to:",
            choices = hospital_choices, selected = hospital_choices[[1]])
```
...
renderPlot({
  ...
  # Create incidence object, with dynamic filter
  age_outbreak <- incidence(
    linelist %>% filter(hospital == input$hospital_choice),
    date_index = date_onset, interval = "week", groups = age_cat)
  
  # plot incidence object as epicurve, with dynamic title
